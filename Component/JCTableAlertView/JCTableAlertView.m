...
@interface JCSelectionCell : UITableViewCell
@property (nonatomic, strong) UIButton *selectionButton;
@property (nonatomic, strong) UILabel*  titleLabel;

@end

@implementation JCSelectionCell
...
@interface JCTableAlertView() <UITableViewDelegate, UITableViewDataSource>
// Date
...
@implementation JCTableAlertView

+ (JCTableAlertView *_Nonnull)initWithTitle:(NSString *)title options:(NSArray *)optionsArray
                               singleSelection:(BOOL)selection
                                 selectedItems:(NSArray *)items
                                     window:(UIWindow *)window
                             completionHandler:(CompleteSelection)handler{
    
    JCTableAlertView* view = [[JCTableAlertView alloc]initWithFrame:window.bounds];
    ...
    [view.tableView registerClass:[JCSelectionCell class] forCellReuseIdentifier:@"Cell"];
...
- (UIButton *)cancelBtn {
    if (!_cancelBtn) {
        ...
        NSLayoutConstraint *splitLeft = [NSLayoutConstraint constraintWithItem:splitView attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:_bottomView attribute:NSLayoutAttributeLeft multiplier:1 constant:0];
        NSLayoutConstraint *splitRight = [NSLayoutConstraint constraintWithItem:splitView attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:_bottomView attribute:NSLayoutAttributeLeft multiplier:1 constant:0];
        NSLayoutConstraint *splitTop = [NSLayoutConstraint constraintWithItem:splitView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:_bottomView attribute:NSLayoutAttributeBottom multiplier:1 constant:0];
        NSLayoutConstraint *splitHeight = [NSLayoutConstraint constraintWithItem:splitView attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeHeight multiplier:1 constant:1.0];
        
        NSArray *constantArrSplit = @[splitLeft, splitRight,splitTop,splitHeight];
        [_bottomView addConstraints:constantArrSplit];
        ...
        NSLayoutConstraint *centerTop = [NSLayoutConstraint constraintWithItem:centerView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:_bottomView attribute:NSLayoutAttributeLeft multiplier:1 constant:0];
        NSLayoutConstraint *centerCenterX = [NSLayoutConstraint constraintWithItem:centerView attribute:NSLayoutAttributeCenterX relatedBy:NSLayoutRelationEqual toItem:_bottomView attribute:NSLayoutAttributeLeft multiplier:1 constant:0];
        NSLayoutConstraint *centerBottom = [NSLayoutConstraint constraintWithItem:centerView attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:_cancelBtn attribute:NSLayoutAttributeBottom multiplier:1 constant:0];
        NSLayoutConstraint *centerWidth = [NSLayoutConstraint constraintWithItem:centerView attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeHeight multiplier:1 constant:1.0];
        
        NSArray *constantArCenter = @[centerTop, centerCenterX,centerBottom,centerWidth];
        [_bottomView addConstraints:constantArCenter];
...
- (void)closeView:(JCTableAlertView *)view {
    [UIView animateWithDuration:0.3 animations:^{
        view.bgView.alpha = 0;
    } completion:^(BOOL finished) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    JCSelectionCell *cell = [tableView dequeueReusableCellWithIdentifier:@"Cell"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    JCSelectionCell *cell = (JCSelectionCell *)[tableView cellForRowAtIndexPath:indexPath];
    if (self.isSingleSelection) {
        ...
        if (self.selectedArray && self.selectedArray.count > 0) {
            ...
            JCSelectionCell *cell = (JCSelectionCell *)[tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:[lastRow integerValue] inSection:0]];
            cell.selectionButton.selected = NO;
        }
        [self.selectedArray removeAllObjects];
        [self.selectedArray addObject:@(indexPath.row)];
